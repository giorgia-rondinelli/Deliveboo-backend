...
@section('content')
    <div>
        ...
        <div>
              <form id="dishForm" action="{{ route('admin.dishes.update', $dish)}}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                {{-- nome del piatto --}}
                <div class="mb-3">
                    <label for="name" class="form-label">Nome</label>
                    <input name="name" type="text" class="form-control" id="name" value="{{ old('name', $dish->name) }}" required>
